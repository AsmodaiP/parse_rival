...
def get_characteristics(id: Union[str, int]) -> Dict[str, str]:
...
def get_rival_info(id: Union[str, int]) -> Dict[str, str]:
    ...
    result = {'description': characteristics['description']}
...
def get_info(id: Union[str, int]) -> Dict[str, str]:
    today = datetime.datetime.now().strftime("%Y-%m-%d")
    yesterday = (datetime.datetime.now() - datetime.timedelta(days=1)).strftime("%Y-%m-%d")
    rival_info  = get_rival_info(61637961)
    with open(f'{today}_{id}.json', 'w') as f:
        json.dump(rival_info, f, indent=4, ensure_ascii=False)
    if os.path.exists(f'{yesterday}_{id}.json'):
        ...
        if rival_info['description'] != yesterday_rival_info['description']:
            ...
            yesterday_rival_info['description'] = replace_specific_words_to_bold(deleted, yesterday_rival_info['description'])
